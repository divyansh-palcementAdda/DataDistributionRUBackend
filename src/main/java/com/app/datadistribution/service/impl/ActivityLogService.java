...
public class ActivityLogService implements IActivityLogService {
	...
	public void logActivity(ActivityType type, String description, String performedBy) {
		ActivityLog log = ActivityLog.builder().activityType(type).description(description)
				.performedBy(performedBy != null ? performedBy : "System").build();
	...
	private ActivityLogDTO toDto(ActivityLog log) {
		return ActivityLogDTO.builder().icon(log.getActivityType().getIcon()).iconBg(log.getActivityType().getIconBg())
				.title(log.getTitle()).desc(log.getDescription()).by("by " + log.getPerformedBy())
				.time(getHumanReadableTime(log.getCreatedAt())).createdAt(log.getCreatedAt()).build();
	}
    ...
    /**
     * Resolves the currently authenticated user's name from Spring Security context.
     * Falls back to "System" if no authentication is found.
     */
	...
	public void logActivity(ActivityType type, String title, String description, String performedBy) {
		ActivityLog logEntry = ActivityLog.builder().activityType(type)
				.title(title != null ? title : type.getDefaultTitle()).description(description).performedBy(performedBy)
				.build();
